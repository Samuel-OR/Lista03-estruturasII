...
void Qpop(lista **l);
void dijkstra(grafo **g, lista *visitados, int partida);
...
void printpath(grafo* g, int node){
    if(g->shortTable[node][1]!=0){
        printpath(g, g->shortTable[node][0]);
    }
    printState(g->states[node]);
    printf("\n");
}
...
void dijkstra(grafo **g, lista *visitados,  int partida){
    ...
    addList(&queue, partida);
    addList(&visitados, partida);
    while(queue){
        cost = (*g)->shortTable[partida][1]+1;
        for(int i=0;i<3;i++){
            if(!searchList(visitados, (*g)->adjacencias[partida][i])){
                if(cost < (*g)->shortTable[(*g)->adjacencias[partida][i]][1] || (*g)->shortTable[(*g)->adjacencias[partida][i]][1]==0){
                    (*g)->shortTable[(*g)->adjacencias[partida][i]][1] = cost;
                    (*g)->shortTable[(*g)->adjacencias[partida][i]][0] = partida;
                    addList(&queue, (*g)->adjacencias[partida][i]);
                }
            }
        }
        /* printf("queue >>");printList(queue);
        printf("\n");
        printf("visited >>");printList(visitados);
        setbuf(stdin,NULL);
        getchar(); */
        Qpop(&queue);
        if(queue)
...
grafo* criaGrafo(int nVertices){
    ...
    g->shortTable = (int**)calloc(sizeof(int*),81);
    for(int i=0;i<81;i++)
        g->shortTable[i] = (int*)calloc(sizeof(int),2);
